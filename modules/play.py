...
@Client.on_message(filters.command(['play', 'play@patricia_robot']) & ~filters.edited & ~filters.bot)
@chat_allowed
@admin_mode_check
                ...
                if filename is None:
                    ...
                    if Config.get('PLAYBACK_FOOTER') not in ['', None]:
                        footer = f"{Config.get('PLAYBACK_FOOTER')}".replace(
                            '\\n', '\n')
                    footer_val = (
                        '\n'+footer) if footer is not None else '\nFor any issues contact @patricia_support'
